...
const STEP_KEYS = [
  { id: "validating", key: "step_validating" },
  { id: "extracting", key: "step_extracting" },
  { id: "analyzing", key: "step_analyzing" },
  { id: "classifying", key: "step_classifying" },
  { id: "recommending", key: "step_recommending" }
]

export function AnalysisWorkspace({ state, verdict, t }: Props) {
    ...
    <div className="flex flex-col min-h-[400px] lg:min-h-[560px] h-fit rounded-xl border border-shield-cyan/15 bg-shield-navy-light/80 p-5 shadow-2xl shadow-shield-cyan/5 backdrop-blur-sm sm:p-6">
      ...
      <div className="mb-6 flex items-center justify-between border-b border-shield-cyan/10 pb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold text-white">{t.workspace_analysis_title}</h2>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="flex flex-col justify-center flex-1">
        ...
        {isIdle && (
          <div className="flex flex-col items-center text-center">
            ...
            <h3 className="text-lg font-semibold text-white">{t.idle_engine_title}</h3>
            <p className="mt-2 max-w-sm text-sm text-shield-muted">
              {t.idle_engine_subtitle}
            </p>
          </div>
        )}
        ...
        {isAnalyzing && (
          <div className="mx-auto w-full max-w-sm space-y-6">
            <div className="text-center">
              ...
              <p className="mt-4 text-sm font-medium text-white">{t.analyzing_threat}</p>
            </div>
            <div className="space-y-3">
              {STEP_KEYS.map((step, index) => {
                const stepIndex = STEP_KEYS.findIndex(s => s.id === state)
                ...
                return (
                  <div key={step.id} className={`flex items-center gap-3 rounded-lg border p-3 transition-colors ${isCurrent ? "border-shield-cyan/30 bg-shield-cyan/10" : isPast ? "border-shield-safe/20 bg-shield-safe/5" : "border-shield-cyan/5 bg-shield-navy/20"}`}>
                    ...
                    <span className={`text-xs font-medium ${isCurrent ? "text-shield-cyan" : isPast ? "text-shield-safe/80" : "text-shield-muted/50"}`}>{t[step.key]}</span>
                  </div>
                )
              })}
            </div>
          ...
          <div className="flex h-full flex-col">
            ...
            <div className="mb-6 grid gap-4 sm:grid-cols-2">
              <div className="rounded-lg border border-shield-cyan/10 bg-shield-navy/40 p-4 text-center">
                <div className="text-[10px] uppercase text-shield-muted">{t.card_threat_score}</div>
                <div className="mt-1 text-2xl font-bold text-white">{verdict.score}<span className="text-sm text-shield-muted">/100</span></div>
              </div>
              <div className="rounded-lg border border-shield-cyan/10 bg-shield-navy/40 p-4 text-center">
                <div className="text-[10px] uppercase text-shield-muted">{t.card_confidence}</div>
            ...
            <div className="mb-4 rounded-lg border border-shield-cyan/20 bg-shield-cyan/5 p-4">
              <div className="mb-2 flex items-center gap-2">
                ...
                <span className="text-xs font-semibold uppercase text-shield-cyan">{t.section_risk_assessment}</span>
              </div>
            ...
            <div className="mt-auto flex items-center justify-between border-t border-shield-cyan/10 pt-4 text-[10px] text-shield-muted">
              <span>{t.label_category}: <strong className="text-white">{verdict.category}</strong></span>
              <span>{t.completed_at} {new Date(verdict.timestamp).toLocaleTimeString()}</span>
            </div>
          </div>
        )}
      </div>
